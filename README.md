...
- **固定间隔模式**：按你设置的分钟数循环触发，例如“每 `30` 分钟、每个活跃母号创建 `1` 个”
- 固定间隔模式会在每个周期遍历所有选中的活跃母号；`count_per_run` 是**每个母号**的数量，不是所有母号合计
- 某个母号失败或触达 Apple 限额时，只跳过该母号本周期任务，其它母号继续执行；下个周期再重新尝试
- 每个母号达到 `750` 个 HME 后会在本地标记为已满；手动、批量和计划任务都不会再调用 Apple 创建接口
- 计划任务配置会持久化到本地 `scheduler_config.json`

加号派生说明：

- 开启后，本地列表 / 复制 / CSV 会把每个实际创建的隐私邮箱额外展示为唯一的 `+1` 变体
...
| `GET /api/agent/account-pool` | 聚合 HME/OpenAI 账号池：母号、base/+tag 邮箱、邮件数、状态、凭证可用性和筛选分页 |
| `GET /api/agent/messages` | 按邮箱、OpenAI 状态、邮件类别、关键词和时间提取邮件元数据 |
| `POST /api/agent/account-pool/credentials` | 为明确的 base 或 `+tag` 邮箱批量生成登录链接（敏感操作） |
